...
class Palestinian(Agent):
    ...
    def react(self,model):
        ...
        if crowding > 30:
            #print('crowding in cell:',crowding)
            self.satisfaction -= 5
        ...
        if govt_action == 'TARG-CONC':
            if action_pos == self.pos:
                #print('conciliatory govt action in same position:',self.pos)
                self.satisfaction += 2
...
class IsraeliGovt(Agent):
    ...
    def act(self, model):  
        ...
        if self.status in ['TARG-CONC', 'TARG-REPR']:
            ...
            #print('targeted govt action in cell:',model.action_pos)
        else:
        ...
        #print('current sustained repression:',model.sust_repr,\
        #    '\n current sustained conciliation:',model.sust_conc)
...
class CounterterrorismModel(Model):
    ...
    """
    def __init__(self, N, height, width, prob_violence=0.005, reactive_lvl = 'mid-low',policy='NONE',\
        discontent='mid', seed=50):
        self.num_agents = N
        self.height = height
        self.width = width
        self.prob_violence = prob_violence
        #print('model prob of violence:',prob_violence)
        self.policy = policy
        #print('model govt policy:',policy)
        self.reactive_lvl = reactive_lvl
        #print('model govt reactive lvl:',reactive_lvl)
        self.discontent = discontent
        #print('model palestinian discontent:',discontent)
        self.current_id = 0
        ...
        print('current id pre-IsraeliGovt',self.current_id)
    ...
    def step(self):
        ...
        #self.datacollector.add_table_row('Deaths',{'step':self.schedule.steps,\
        #    'deaths':len(self.deaths)})

        #print('total number of agents before births/deaths:',self.num_agents)
        # remove dead agents from model
        #print('num deaths if any:',len(self.deaths))
        ...
        #print('new births to add:',new_births)
        ...
        #print('total number of agents in model after births and deaths:',\
        #    self.num_agents)
